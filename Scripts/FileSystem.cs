...
public class FileSystem : MonoBehaviour
{
    ...
    private string DirectoryLol = "";
    private string OcksDirectry = "";
    ...
    private void Awake()
    {
        ...
        WriteFile($"{OcksDirectry}\\Ocks_Games_Owned.txt", "", false);
        GameName = $"Ocks Tools v{RandomFunctions.Instance.GameVer}";
        var s = ReadFile($"{OcksDirectry}\\Ocks_Games_Owned.txt");
        if (!s.Contains(GameName))
        {
            s += $"{GameName}\n";
            WriteFile($"{OcksDirectry}\\Ocks_Games_Owned.txt", s, true);
        }
        var pp = new bool[37];
